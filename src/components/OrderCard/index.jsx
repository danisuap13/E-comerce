...
const OrderCard = (props) => {
	...
	const { id, title, image , price, value, description, handleDelete, isMyOrder} = props;
	let renderTrashIcon;
	let renderValue;
	let renderImage;
	if(handleDelete) {
	...
	if(isMyOrder) {
		renderValue = <p className='mx-3 text-md font-light border border-black rounded-lg p-2'>{value}</p>
		renderImage = <img className="w-full h-full rounded-lg object-cover" src={image} alt={title} />
	}
	else {
		renderValue = <p className='mx-3 text-md font-light border border-black rounded-lg p-2 hover:cursor-pointer hover:border-white' onClick={() => context.updateCardValue(id)} >{value}</p>
	...
	return(
		<div className='flex justify-between items-center mb-3 border border-gray-300 rounded-lg'>
			...
			<div className='flex items-center gap-1'>
				<p className="text-lg font-medium ml-2 pr-2">${(price*value).toFixed(2)}</p>
				{renderTrashIcon}
			</div>
		</div>
	);
