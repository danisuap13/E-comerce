	...
	return(
		<div className={`${context.isQuantityMenuOpen ? 'flex' : 'hidden'} flex-col h-full w-full fixed border border-black rounded-lg bg-gray-400 opacity-90 top-0 justify-center`}>
			...
			<div className='hover:cursor-pointer self-end'>
				<XMarkIcon className='h-6 w-6 text-red-500' onClick={() => context.closeQuantityMenu()}/>
				</div>
				<div className='flex justify-center'>
				<div className='hover:cursor-pointer ml-2'>
						<ChevronDoubleLeftIcon className='h-6 2-6 text-black select-none' onClick={() => {
							context.quantityCounter > 0 ? context.setQuantityCounter(context.quantityCounter - 1) : context.quantityCounter}}/>
				</div>
					...
					<div className='hover:cursor-pointer'>
						<ChevronDoubleRightIcon  className='mr-2 h-6 2-6 text-black select-none' onClick={() => {
							context.quantityCounter < 20 ? context.setQuantityCounter(context.quantityCounter + 1) : context.quantityCounter
						}} />
					</div>
				</div>
